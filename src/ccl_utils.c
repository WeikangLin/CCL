...
#define LMB_GAMMA1 2.6789385347077476336556 //Gamma(1/3)
#define LMB_GAMMA2 1.3541179394264004169452 //Gamma(2/3)
#define LMB_ROOTPI12 21.269446210866192327578 //12*sqrt(pi)

static double ccl_sph_bessel_j(int l,double x)
{
  double jl;
  double ax=fabs(x);
  double ax2=x*x;
  ...
  if(l<7) {
	...
	jl=(LMB_GAMMA1*secb+beta*LMB_GAMMA2*sec2b
	    -(beta2/18-1.0/45.0)*beta*sx*secb*LMB_GAMMA1
	    -((beta2-1)*beta2/36+1.0/420.0)*sx*sec2b*LMB_GAMMA2
	    +(((beta2/1620-7.0/3240.0)*beta2+1.0/648.0)*beta2-1.0/8100.0)*sx2*secb*LMB_GAMMA1
	    +(((beta2/4536-1.0/810.0)*beta2+19.0/11340.0)*beta2-13.0/28350.0)*beta*sx2*sec2b*LMB_GAMMA2
	    -((((beta2/349920-1.0/29160.0)*beta2+71.0/583200.0)*beta2-121.0/874800.0)*
	      beta2+7939.0/224532000.0)*beta*sx2*sx*secb*LMB_GAMMA1)*sqrt(sx)/LMB_ROOTPI12;
      }
    }
  }
  if((x<0)&&(l%2!=0)) jl=-jl;
